...
document.querySelector("#btn-play").addEventListener("click", () => {
  if (name_input.value.trim().length > 0) {
    const gameId = uuid.v4(); // 生成一个新的 UUID
    localStorage.setItem("currentGameId", gameId); // 将 UUID 保存到 localStorage
...
document.querySelector("#btn-continue").addEventListener("click", () => {
  if (name_input.value.trim().length > 0) {
    // TODO: 替换下面的 getGameUuid 方法以获取真实的游戏UUID
    const gameUuid = getGameUuid(); // 这个方法应该返回要加载的游戏的UUID
    loadGameFromServer(gameUuid); // 从服务器加载游戏状态
    startGame();
  } else {
    name_input.classList.add("input-err");
    setTimeout(() => {
...
function saveGameToServer(gameState) {
  const gameId = localStorage.getItem("currentGameId"); // 从 localStorage 获取当前游戏的 UUID
  const gameData = {
    uuid: gameId,
  ...
  fetch(`https://sudoku-shadow.vercel.app/load-game/${uuid}`)
    ...
    .then((data) => {
      ...
      if (data && data.game_state) {
        try {
          const loadedGameState = JSON.parse(data.game_state);
          console.log("Parsed loaded game state:", loadedGameState);

          if (
            !loadedGameState.su ||
            typeof loadedGameState.su !== "object" ||
            !Array.isArray(loadedGameState.su.original) ||
            !Array.isArray(loadedGameState.su.question) ||
            !Array.isArray(loadedGameState.su.answer)
          ) {
            console.error(
              "Invalid game state loaded from server:",
              loadedGameState
            );
            return;
          }

          su = loadedGameState.su;
          su_answer = loadedGameState.su.answer;
          seconds = loadedGameState.seconds;
          level_index = loadedGameState.level;
          updateGameDisplay();
        } catch (e) {
          console.error("Error parsing game state:", e);
        }
      } else {
        console.error("Game state data is missing or invalid:", data);
      }
    })
    .catch((error) => {
...
function updateGameDisplay() {
  console.log("Updating game display with loaded game state");
  if (
    !su ||
    !su_answer ||
    !Array.isArray(su.question) ||
    !Array.isArray(su_answer)
  ) {
    console.error("Loaded game state is invalid:", su);
    return;
  }
